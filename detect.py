...
def onemincheck():
    today = date.today()
    tf = open('counttxt/' + today.isoformat()+'.txt', 'a')
    global count
    global val2
    count = ccc
    savecountcar = count - val2
    now = datetime.now()
    nowtime = str(now.hour) + ' : ' + str(now.minute)
    tf.write(nowtime + '\t' + str(savecountcar) + '\n')
    val2 = count
    tf.close()
    threading.Timer(60, onemincheck).start()
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        for i, det in enumerate(pred):  # detections per image
            ...
            if det is not None and len(det):
                ...
                for *xyxy, conf, cls in det:
                    ...
                    if(names[int(cls)] == 'person'):
                        t3 = time.time()
                        if(t3 - t0 < 16 and t3 - t0 > 15.5):
                            first_check = (xyxy[1])
                            print("aaaaaaaaaaaaa")
                        if(t3 - t0 < 31 and t3 - t0 > 30.5):
                            second_check = (xyxy[1])
                            #ratio = 0.0003 / float((second_check - first_check))

                    tx = open('ratio.txt', 'r')
                    ratio = float(tx.readline())
                    tx.close()
            ...
            for j in range(len(tracker.tracker_list)):
                ...
                '''
                if(c_y > 300 and c_y < 330):
                    if(thumb < 5):
                      sum1 = sum1 + abs(y2-y1)
                      thumb = thumb + 1
                      ratio =  0.002 / (sum1/thumb)
                '''

                if(diff > 0 and diff < 30):
                    try:
                        if(globals()['ce' + str(tracker.tracker_list[j].tracking_id)].save_count == 0):
                            tf = open('speedtxt/' + 'speed.txt', 'a')
                            sp = globals()['ce' + str(tracker.tracker_list[j].tracking_id)].cal()
                            tf.write(str(tracker.tracker_list[j].tracking_id) + '\t' + str(sp) + '\n')
                            tf.close()
                            globals()['ce' + str(tracker.tracker_list[j].tracking_id)].save()
            
                    except KeyError:
                        pass
        
                for q in range(len(cross_car)):
                    for k in range(len(idc)):
                        if(cross_car[q] == idc[k]):
                            continue
                        elif(cross_car[q] != idc[k]):
                            cross_car.append(idc[k])
                            cross_car = list(set(cross_car))
                ccc = len(cross_car) - 1

                ################################카운트,속도 표시##################################################################
                font=cv2.FONT_HERSHEY_SIMPLEX
                putcount = str(ccc) 
                cv2.putText(im0,putcount,(50,50),font,1,(0,0,255),4)
                #########################################################################################################################################################
...
if __name__ == '__main__':
    ...
    parser.add_argument('--weights', nargs='+', type=str, default='car_epoch100.pt', help='model.pt path(s)')
